...
export async function GET() {
  try {
    ...
    if (error) {
      console.error('[GET /api/recipe-banners] Error:', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }

    return NextResponse.json({ data: data || [] })
  } catch (err) {
    console.error('[GET /api/recipe-banners] Unexpected error:', err)
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error) {
      console.error('[POST /api/recipe-banners] Error:', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }

    return NextResponse.json({ data }, { status: 201 })
  } catch (err) {
    console.error('[POST /api/recipe-banners] Unexpected error:', err)
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'An unexpected error occurred' },
      { status: 500 }
    )
