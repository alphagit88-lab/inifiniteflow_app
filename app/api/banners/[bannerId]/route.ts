...
export async function DELETE(_: Request, { params }: RouteContext) {
  ...
  try {
    ...
    if (error) {
      console.error('[DELETE /api/banners/[bannerId]] Error:', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (err) {
    console.error('[DELETE /api/banners/[bannerId]] Unexpected error:', err)
...
export async function PATCH(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    if (error) {
      console.error('[PATCH /api/banners/[bannerId]] Error:', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }
    ...
    console.error('[PATCH /api/banners/[bannerId]] Unexpected error:', err)
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'An unexpected error occurred' },
      { status: 500 }
    )
